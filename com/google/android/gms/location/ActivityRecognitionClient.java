...
public class ActivityRecognitionClient extends GoogleApi<Api.ApiOptions.NoOptions> {
  ...
  public Task<Void> removeActivityTransitionUpdates(PendingIntent paramPendingIntent) {
    return doWrite(TaskApiCall.builder().run(new zze(paramPendingIntent)).build());
  }
  
  public Task<Void> removeActivityUpdates(PendingIntent paramPendingIntent) {
    return doWrite(TaskApiCall.builder().run(new zzd(paramPendingIntent)).build());
  }
  ...
  public Task<Void> requestActivityTransitionUpdates(ActivityTransitionRequest paramActivityTransitionRequest, PendingIntent paramPendingIntent) {
    return doWrite(TaskApiCall.builder().run(new zzc(paramActivityTransitionRequest, paramPendingIntent)).build());
  }
  
  public Task<Void> requestActivityUpdates(long paramLong, PendingIntent paramPendingIntent) {
    return doWrite(TaskApiCall.builder().run(new zzb(paramLong, paramPendingIntent)).build());
  }
...
/* Location:              D:\code\BluedHook\classes.dex\com.soft.blued5032032-dex2jar.jar!\com\google\android\gms\location\ActivityRecognitionClient.class
